...
export const Route = createFileRoute("/competitions/$slug")({
  ...
  head: ({ params }) => ({
    meta: [
      { title: `Competition — Lucky Git Comps` },
      { name: "description", content: `Enter to win. Free postal entry available.` },
  ...
  notFoundComponent: () => (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="mx-auto max-w-2xl px-4 py-24 text-center flex-1">
        ...
        <p className="mt-2 text-muted-foreground">Either sold out or never existed. Try one that's live.</p>
...
function CompDetail() {
  ...
  const c = data as DbCompetition; // loader guarantees non-null
  ...
  const pct = Math.round(((c.totalTickets - c.ticketsAvailable) / c.totalTickets) * 100);
  const odds = Math.max(1, Math.round(c.totalTickets / Math.max(1, c.totalTickets - c.ticketsAvailable)));

  const canProceed = answered === c.skillQuestion.correct;
  const displayNumbers = picker === "manual" ? picked.size : qty;

  // Show a manageable slice of the 500 numbers but let user page through them.
  ...
  const handleReserve = async () => {
    ...
    try {
      ...
      sessionStorage.setItem(
        "lgc:reservation",
        JSON.stringify({ token, slug: c.slug, numbers, expires: Date.now() + 15 * 60_000 }),
      );
          ...
          <div className="lg:col-span-2">
            ...
            <div className="mt-5">
              <div className="flex justify-between text-sm font-mono tabular-nums mb-1.5">
                <span>{shortNumber(c.totalTickets - c.ticketsAvailable)} / {shortNumber(c.totalTickets)} sold</span>
            ...
            <div className="mt-6 rounded-2xl bg-card border-2 border-white/5 p-4">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-1 text-xs font-mono tabular-nums">
                      ...
                      <button
                        disabled={numberPage >= pageCount - 1}
                        onClick={() => setNumberPage((p) => Math.min(pageCount - 1, p + 1))}
                        className="px-2 py-1 rounded border border-white/10 disabled:opacity-30"
                      >→</button>
                    </div>
                  </div>
                  ...
                  <div className="mt-2 flex items-center justify-between text-xs text-muted-foreground font-mono tabular-nums">
                    ...
                    {picked.size > 0 && (
                      <button onClick={() => setPicked(new Set())} className="underline">clear</button>
                    )}
                  </div>
                </div>
              )}
              ...
              {reserveError && (
                <div className="mt-3 flex items-start gap-2 text-xs text-hot bg-hot/10 border border-hot/30 rounded-lg p-2">
                  ...
                  <span>{reserveError} Refresh and try again.</span>
                </div>
              )}
              ...
              <div className="mt-4 pt-4 border-t border-white/10 flex items-center justify-between">
                ...
                <Button
                  ...
                  disabled={displayNumbers === 0 || reserving}
                >
                  Enter now
                </Button>
              </div>
        ...
        <section className="mt-12 grid gap-8 lg:grid-cols-3">
          ...
          <aside className="rounded-2xl bg-gold/10 border-2 border-gold/40 p-5">
            ...
            <p className="mt-2 text-sm text-foreground/80">
              Every competition can be entered by post. Same odds, no purchase required.
            </p>
      ...
      {skillOpen && (
        ...
        >
          <div className="bg-background rounded-3xl border-2 border-white/10 w-full max-w-md p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            {reserveError && (
              <div className="mt-3 text-xs text-hot">{reserveError}</div>
            )}
