...
import { POOL_CAP_LINE, TIMING_LINE } from "@/lib/promises";
...
export const Route = createFileRoute("/how-it-works")({
  ...
  component: () => (
    <StaticPage kicker="The mechanics" title="How it works">
      ...
      <p>Choose a competition, decide how many tickets you want, and either use Lucky Dip or hand-pick your numbers. Tickets start from £1 — some prizes go for a fiver a pop. {POOL_CAP_LINE}</p>
      <h2>2. Answer the skill question</h2>
      <p>Every competition has a genuine question of skill. Answer correctly and your tickets are entered in the draw. Answer incorrectly and your tickets are recorded as non-qualifying — you have paid for entry but you are not in the draw. This is stated unmissably on the payment screen.</p>
      <h2>3. We auto-draw from correct entries</h2>
      <p>When the timer hits zero (or the last ticket sells), our system picks a winner deterministically from the pool of correct entries only, using a hash-then-reveal method. The winning number, sold-ticket count and qualifying-pool size are published on the <a href="/past-draws">Past Draws</a> log for anyone to check.</p>
      <p>{TIMING_LINE}</p>
