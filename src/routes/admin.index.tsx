  ...
  return (
    <div className="min-h-screen flex flex-col bg-cream">
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 w-full flex-1">
        ...
        <div className="mt-8 rounded-2xl bg-white border-2 border-ink/5 overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-cream">
                <tr className="text-left text-xs uppercase tracking-wider text-ink/60">
              ...
              <tbody>
                {COMPETITIONS.map((c) => (
                  <tr key={c.slug} className="border-t border-ink/5">
                    ...
                    <td className="p-3">
                      <div className="flex gap-1">
                        <button className="p-2 rounded-lg hover:bg-cream" title="Duplicate"><Copy className="h-4 w-4" /></button>
                        <button className="p-2 rounded-lg hover:bg-cream" title="Draw winner"><Trophy className="h-4 w-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
...
function Stat({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className={`rounded-2xl p-5 border-2 ${accent ? "bg-clover text-cream border-clover" : "bg-white border-ink/5"}`}>
      <div className={`text-xs uppercase tracking-widest font-bold ${accent ? "text-cream/70" : "text-ink/60"}`}>{label}</div>
